...
|---|---|
| 평균 휘도 | 0.067 → **0.309** |
| RMS 콘트라스트 | 0.060 → **0.136** |
| 선명도 (Laplacian var) | 652 → **8,737** |
...
**프레임 90.6% 감축**, 처리 시간은 영상 길이의 0.20배 (CPU). 간판이 있는 장면 4개는
...
> 위 수치는 **합성 검증 영상**(53초, 정적 구간·간판·모션 블러·실내 저조도·재방문 컷·과노출
> 하늘을 의도적으로 심음) 기준이다. 실제 YouTube 영상은 저작권·재현성 문제로 레포에 넣을 수
> 없어 재현 가능한 합성 영상을 썼다. 실영상에서의 장소 추출 재현율 검증은 아직 하지 않았다.
...
- [x] **비전 프론트엔드** — 화질 지표 · 미니 ISP · 문자 saliency · 장면 dedup · 벤치마크 (테스트 63개)
- [ ] **Phase 1** DB 모델 & DTO
- [ ] **Phase 2** 프론트엔드 뼈대
...
| 문서 | 내용 |
|---|---|
| [docs/vision-frontend.md](docs/vision-frontend.md) | 비전 프론트엔드 설계 근거 · 측정값 · 한계 |
