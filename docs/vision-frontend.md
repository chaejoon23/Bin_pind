...
`truth.json`은 손으로 쓴다 — 구간과 그 구간에 보이는 간판 문자열만 있으면 된다.

```json
{"places": [{"place_id": "p1", "sign_text": "블루보틀 성수",
             "dark": false, "segments": [[12.0, 24.0], [131.0, 138.0]]}]}
```
...
> 잃지 않는다"**는 뜻이고, "실제 브이로그에서 잃지 않는다"는 뜻이 아니다. 다만 `validate_recall.py`는
> 영상과 정답 JSON만 있으면 실사 영상에도 그대로 돌아간다 (`--video` / `--ground-truth`).
> 직접 찍은 영상에 대한 검증이 다음 작업이다.

## 남은 일 / 알려진 한계

- **실사 촬영 영상 검증 미완.** 장소 보존률 하네스(`validate_recall.py`)는 만들었고 합성
  장면에서 오라클 동률(8/8)을 확인했지만, 입력이 합성 영상이다. 직접 찍은 영상 + 손으로 만든
  정답 목록으로 같은 하네스를 돌리는 것이 다음 작업이다.
