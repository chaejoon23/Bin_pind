...
- [ ] 테스트 3편 fetch → 제외 기준·구간 → 라벨 커밋(결과 보기 전) → 고정 명령 실행 → 규칙대로 기본값 결정
...
2. **비전 프론트엔드 선별기 테스트** — `text_nms` 구현·사전 등록 완료. `youtube_fetch.py --split test` →
   라벨 커밋 → `PREREGISTRATION.md` 의 고정 명령 실행 → 채택 규칙대로 기본값 결정
